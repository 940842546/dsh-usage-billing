...
function emptyStats() {
  return {
    version: 1,
    updatedAt: 0,
    meta: { liveSince: 0, lastBackfillAt: 0, lastBackfillSessions: 0, lastBackfillFound: 0, sessionAttribution: false, schemaVersion: 4 },
    total: bucket(),
...
/** 高峰 = 工作日（周一至周五）且时间落在任一峰段内；周末全天空闲 */
function isPeakHour(hour, peak, weekday) {
...
function bandOf(ts, cfg) {
  ...
  return isPeakHour(bp.hour, cfg.peakHours, bp.weekday) ? 'afterPeak' : 'afterOffPeak';
}
...
export default {
  ...
  async apply(ctx) {
    ...
    // 主路由 /usage-billing；兼容别名 /usage-stats（过渡期保留，供旧客户端/书签）
    ...
    ctx.effect(() => webServer.register({
      ...
      path: '/usage-stats',
      handler: statsRouteHandler,
    }), 'usage-billing: legacy stats route alias');
    ...
    // 主名 usage_billing；别名 usage_stats（旧对话/指令兼容，行为完全一致）
    ...
    ctx.effect(() => tools.register(defineTool({
      name: 'usage_stats',
      description: usageToolDesc + '（usage_billing 的兼容别名）',
      parameters: {
        scope: { type: 'string', enum: ['today', 'month', 'all'], description: '统计范围：today=今日，month=本月，all=累计（默认）' },
      },
      output: usageToolOutput,
      execute: usageToolExecute,
    })), 'usage-billing: usage_stats legacy alias');
    ...
    if (stats.total.calls === 0 || stats.meta.sessionAttribution !== true || stats.meta.schemaVersion !== 4) {
      stats = emptyStats();
      stats.meta.liveSince = Date.now();
