...
function emptyStats() {
  return {
    version: 1,
    updatedAt: 0,
    meta: { liveSince: 0, lastBackfillAt: 0, lastBackfillSessions: 0, lastBackfillFound: 0, sessionAttribution: false, schemaVersion: 9 },
    total: bucket(),
...
function bandOf(ts, cfg) {
  ...
  const peak = isPeakHour(bp.hour, cfg.peakHours, bp.weekday, from, ts);
...
export default {
  ...
  async apply(ctx) {
    ...
    if (stats.total.calls === 0 || stats.meta.sessionAttribution !== true || stats.meta.schemaVersion !== 9) {
      stats = emptyStats();
      stats.meta.liveSince = Date.now();
