...
function emptyStats() {
  return {
    version: 1,
    updatedAt: 0,
    meta: { liveSince: 0, lastBackfillAt: 0, lastBackfillSessions: 0, lastBackfillFound: 0, sessionAttribution: false, schemaVersion: 7 },
    total: bucket(),
...
export default {
  ...
  async apply(ctx) {
    ...
    function recordUsage(modelName, usage, ts, sessionId) {
      ...
      // 官方路由窗口（V4.1 Flash 上线 ~ V4.1 Pro 上线）：V4 Pro 请求被路由到 V4.1 Flash 并按 Flash 价计费。
      // 开关默认开：flashSepMs 起 pro 档按同段 flash 价格计，统计身份仍记 pro。
      const routed = model === 'pro' && cfg.proRoutesToFlash !== false && ts >= (Number(cfg.flashSepMs) > 0 ? cfg.flashSepMs : FLASH_SEP_MS);
    ...
    if (stats.total.calls === 0 || stats.meta.sessionAttribution !== true || stats.meta.schemaVersion !== 7) {
      stats = emptyStats();
      stats.meta.liveSince = Date.now();
