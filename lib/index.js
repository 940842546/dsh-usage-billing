...
function emptyStats() {
  return {
    version: 1,
    updatedAt: 0,
    meta: { liveSince: 0, lastBackfillAt: 0, lastBackfillSessions: 0, lastBackfillFound: 0, sessionAttribution: false, schemaVersion: 8 },
    total: bucket(),
...
function sanitizeConfig(raw) {
  ...
  return {
    ...
    proRoutesToFlash: typeof raw.proRoutesToFlash === 'boolean' ? raw.proRoutesToFlash : true,
  };
}
...
export default {
  ...
  async apply(ctx) {
    ...
    if (stats.total.calls === 0 || stats.meta.sessionAttribution !== true || stats.meta.schemaVersion !== 8) {
      stats = emptyStats();
      stats.meta.liveSince = Date.now();
