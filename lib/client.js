	...
	factory: (require) => {
		...
		const DICTS = {
			zh: {
				...
				'ov.cny': '¥ CNY',
				'ov.usd': '$ USD',
				'ov.note': '按 DeepSeek 官方定价计费（元/百万 tokens），区分 2026-08-17 调价前后及高峰/空闲时段，仅统计已定价模型；汇率 1 USD = 6.7878 CNY（2026-08-14 中间价）。',
				'ov.sessions': '总会话数',
			...
			en: {
				...
				'ov.cny': '¥ CNY',
				'ov.usd': '$ USD',
				'ov.note': 'Priced with official DeepSeek rates (CNY / 1M tokens) across the pre/post 2026-08-17 peak and off-peak bands; priced models only. Exchange rate: 1 USD = 6.7878 CNY (2026-08-14 central parity).',
...
.usg-toast { background: var(--dsw-alias-bg-layer-1); border: 1px solid var(--dsw-alias-border-l1); border-left: 3px solid #f59e0b; border-radius: 8px; padding: 10px 14px; font-size: 13px; color: var(--dsw-alias-label-primary); box-shadow: 0 6px 20px rgba(0,0,0,0.18); animation: usg-toast-in 0.2s ease-out; }
...
`;
		...
		function AlertToaster() {
				...
				const poll = () => {
					fetchStats().then((d) => {
						...
						setToasts((prev) => {
							...
							const fresh = items.filter((t) => !seen.has(t.id));
							return fresh.length > 0 ? prev.concat(fresh).slice(-4) : prev;
						});
					}).catch(() => {});
				};
				poll();
			...
			react.useEffect(() => {
				...
				const timer = window.setTimeout(() => setToasts((prev) => prev.slice(1)), 6000);
				return () => window.clearTimeout(timer);
			}, [toasts]);
			if (toasts.length === 0) return null;
			return el('div', { className: 'usg-toaster' },
				toasts.map((t) => el('div', { key: t.id, className: 'usg-toast' + (t.level >= 2 ? ' usg-toast-over' : '') }, t.text)),
			);
		}
