...
`;
		...
		function cellTitle(label, b) {
			return label + ' · 输入 ' + fmtTokens(b.input) + ' / 输出 ' + fmtTokens(b.output) + ' tokens · ' + fmtCost(b.cost) + ' · ' + b.calls + ' 次调用';
		}
		...
		function dayGridHeatMap(keys, buckets, getBucket, dense) {
		...
		function hourHeatMap(s) {
			...
			for (let h = 0; h < 24; h++) {
				...
				cells.push(el('div', { className: 'usg-hm-col', key: h },
					el('div', {
						className: 'usg-hm-cell' + (lvl > 0 ? ' usg-hm-' + lvl : ''),
						title: cellTitle(today + ' ' + p2(h) + ':00', b),
					}),
				));
			}
			const hourLabels = [0, 6, 12, 18, 23].map((h) =>
				el('div', { className: 'usg-hm-label', key: h, style: { width: 12 } }, String(h)),
			);
		...
		function heatMap(s, range) {
			const getDayBucket = (k) => (s.byDay && s.byDay[k]) || EMPTY_BUCKET;
			if (range === 'day') {
				if (!s.byHour || Object.keys(s.byHour).length === 0) {
					return el('div', { className: 'usg-hm-wrap' },
						el('div', { className: 'usg-muted' }, '小时级数据将在宿主重启后回填（自动重建统计）'),
					);
				}
				return hourHeatMap(s);
			}
			if (range === 'week') {
				const keys = [];
				for (let i = 6; i >= 0; i--) keys.push(beijingDateKey(-i));
				return dayGridHeatMap(keys, null, getDayBucket, false);
			}
			if (range === 'year') {
				const keys = [];
				for (let i = 364; i >= 0; i--) keys.push(beijingDateKey(-i));
				return dayGridHeatMap(keys, null, getDayBucket, true);
			}
			if (range === 'all') {
				...
				let cur = days[0];
				const today = beijingDateKey(0);
				while (cur <= today) { keys.push(cur); cur = nextDateKey(cur); }
				return dayGridHeatMap(keys, null, getDayBucket, true);
			}
			// 默认：月（近 30 天）
			const keys = [];
