	...
	factory: (require) => {
		...
		const DICTS = {
			zh: {
				...
				'ov.totalCost': '总费用',
				'ov.calls': '调用次数',
				'ov.outputTokens': '输出 tokens',
				'ov.bandSection': '计费分段占比',
				'ov.heatmapSection': '用量热力图',
				'ov.topSessions': '会话消费 Top 5',
...
/* 主界面：悬浮汇总面板 */
.usg-ov { position: fixed; left: 12px; bottom: 100px; width: 340px; max-width: calc(100vw - 32px); max-height: calc(100vh - 140px); overflow-y: auto; pointer-events: auto; background: var(--dsw-alias-bg-layer-1); border: 1px solid var(--dsw-alias-border-l1); border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.2); padding: 12px; display: flex; flex-direction: column; gap: 10px; font-size: 13px; color: var(--dsw-alias-label-primary); }
...
.usg-cfg-check { display: flex; align-items: center; gap: 6px; font-size: 12px; color: var(--dsw-alias-label-secondary); cursor: pointer; }
...
`;
		...
		function bandStack(s) {
			...
			const segs = BAND_KEYS.map((bk, i) => {
				...
				return el('div', {
					...
					style: { width: pct + '%' },
					title: bandLabel(bk) + '：' + fmtCost(bandCosts[i]),
				});
			});
		...
		function dayGridHeatMap(keys, getBucket, tip, scrollRef) {
		...
		// 「日」视图：今天 24 小时
		function hourHeatMap(s, tip, scrollRef) {
		...
		function heatMap(s, range, tip, scrollRef) {
			...
			if (range === 'day') {
				...
				return hourHeatMap(s, tip, scrollRef);
			}
			if (range === 'week') {
				const keys = [];
			...
			if (range === 'all') {
				...
				return el('div', null,
					...
					dayGridHeatMap(keys, getDayBucket, tip, scrollRef),
				);
			}
			// 默认：月（近 30 天）
			const keys = [];
		...
		function BalanceCard(balance, onRefresh) {
			...
			const b = balance;
			if (b.status === 'disabled') return null;
			if (b.status === 'unconfigured') {
				return el('div', { className: 'usg-balance usg-balance-muted' }, T('balance.unconfigured'));
			}
			if (b.status === 'loading') {
				return el('div', { className: 'usg-balance usg-balance-muted' }, T('balance.loading'));
			}
			if (b.status === 'error') {
				return el('div', { className: 'usg-balance usg-balance-error' }, T('balance.error') + T('common.detail', { msg: b.error }));
			}
			const d = b.data;
			if (!d) return null;
		...
		function draftFromConfig(c) {
			...
			return {
				...
				balanceEnabled: src.balanceEnabled !== false,
			...
			const onSave = async () => {
				try {
					const res = await fetch('/usage-stats?action=config', {
						...
						body: JSON.stringify(buildConfigFromDraft(draft)),
					});
			...
			return el('details', { className: 'usg-pricing' },
				...
				el('div', { className: 'usg-cfg' },
					el('label', { className: 'usg-cfg-check' },
						el('input', { type: 'checkbox', checked: !!draft.balanceEnabled, onChange: (e) => set('balanceEnabled', e.target.checked) }),
						el('span', null, T('cfg.balanceEnabled')),
					),
		...
		function FooterButton(props) {
			...
			return el('button', {
				className: 'usg-fab',
				title: open ? T('fab.closeTooltip') : T('fab.tooltip'),
				onClick: () => overlayStore.toggle(),
				'aria-pressed': open,
				style: props.wide ? null : { justifyContent: 'center', padding: '0 6px' },
			},
				el('span', null, open ? T('fab.labelOpen') : T('fab.label')),
					...
					: el('div', null,
							el('div', { className: 'usg-ov-stats' },
								el('div', { className: 'usg-ov-stat' },
									el('div', { className: 'usg-ov-stat-label' }, T('ov.totalCost')),
									el('div', { className: 'usg-ov-stat-value usg-cost' }, fmtCost(s.total.cost)),
								),
								el('div', { className: 'usg-ov-stat' },
									el('div', { className: 'usg-ov-stat-label' }, T('ov.calls')),
									el('div', { className: 'usg-ov-stat-value' }, String(s.total.calls)),
								),
								el('div', { className: 'usg-ov-stat' },
									el('div', { className: 'usg-ov-stat-label' }, T('ov.outputTokens')),
							...
							el('div', null,
								el('div', { className: 'usg-section' }, T('ov.bandSection')),
								bandStack(s),
							),
							el('div', null,
								el('div', { className: 'usg-head' },
									el('div', { className: 'usg-section' }, T('ov.heatmapSection')),
									el(RangeChips, { value: range, onChange: setRange }),
								),
								heatMap(s, range, tip, scrollRef),
							),
							el('div', null,
								el('div', { className: 'usg-section' }, T('ov.topSessions')),
								sessionBars(s, 5, false),
							),
							el('div', { className: 'usg-muted' }, T('ov.footerHint')),
						),
			);
		}
		...
		function UsagePanel() {
			...
			return el('div', { className: 'usg-panel' },
				...
				BalanceCard(data.balance, refreshBalance),
				el('div', null,
