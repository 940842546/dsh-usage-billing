	...
	factory: (require) => {
		...
		const DICTS = {
			zh: {
				...
				'ov.cny': '¥ CNY',
				'ov.usd': '$ USD',
				'ov.note': '按 DeepSeek 官方定价计费（元/百万 tokens），区分 2026-08-17 调价前后及高峰/空闲时段，仅统计已定价模型；汇率 1 USD = 7.2 CNY。',
				'ov.sessions': '总会话数',
			...
			en: {
				...
				'ov.cny': '¥ CNY',
				'ov.usd': '$ USD',
				'ov.note': 'Priced with official DeepSeek rates (CNY / 1M tokens) across the pre/post 2026-08-17 peak and off-peak bands; priced models only. Exchange rate: 1 USD = 7.2 CNY.',
		...
		function fmtMoney(n) {
			const v = Number(n) || 0;
			return CUR === 'usd' ? '$' + (v / 7.2).toFixed(4) : '¥' + v.toFixed(4);
		}
...
.usg-ov-table { width: 100%; border-collapse: collapse; font-size: 12px; }
...
`;
		...
		function OverlayPanel() {
			...
			const [cur, setCur] = react.useState('cny');
			const tip = useTip();
				...
				el('div', { className: 'usg-ov-head' },
					...
					el('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
						el('div', { className: 'usg-ov-cur' },
							el('button', { className: 'usg-ov-cur-btn' + (cur === 'cny' ? ' usg-ov-cur-on' : ''), onClick: () => setCur('cny'), 'aria-pressed': cur === 'cny' }, T('ov.cny')),
							el('button', { className: 'usg-ov-cur-btn' + (cur === 'usd' ? ' usg-ov-cur-on' : ''), onClick: () => setCur('usd'), 'aria-pressed': cur === 'usd' }, T('ov.usd')),
						),
				...
				!s
					...
					: el('div', null,
							el('div', { className: 'usg-ov-stats' },
								...
								ovStat(T('ov.totalTokens'), fmtNum(total.input)),
							...
							el('div', null,
								...
								sessionRows.length > 0
									? el('table', { className: 'usg-ov-table' },
											tableHead([
												{ t: T('th.title') }, { t: T('ov.uncached'), num: true }, { t: T('ov.cached'), num: true }, { t: T('ov.output'), num: true }, { t: T('th.calls'), num: true }, { t: T('ov.cost'), num: true },
											]),
