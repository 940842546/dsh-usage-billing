...
.usg-hm-wrap { display: flex; flex-direction: column; gap: 4px; padding: 10px 2px 2px; border: 1px solid var(--dsw-alias-border-l1); border-radius: 8px; background: var(--dsw-alias-bg-layer-2); }
...
.usg-hm-cell { width: 12px; height: 12px; border-radius: 2px; background: var(--dsw-alias-bg-base, transparent); border: 1px solid var(--dsw-alias-border-l1); box-sizing: border-box; }
.usg-hm-cell.usg-hm-1 { background: var(--dsw-alias-brand-primary); border-color: transparent; opacity: 0.25; }
.usg-hm-cell.usg-hm-2 { background: var(--dsw-alias-brand-primary); border-color: transparent; opacity: 0.45; }
.usg-hm-cell.usg-hm-3 { background: var(--dsw-alias-brand-primary); border-color: transparent; opacity: 0.7; }
.usg-hm-cell.usg-hm-4 { background: var(--dsw-alias-brand-primary); border-color: transparent; opacity: 1; }
...
`;
		...
		// GitHub 风格活跃热力图：周列 × 7 行，按当日费用分 4 档着色
		function heatMap(s, days) {
			const count = days || 14;
			const keys = [];
			for (let i = count - 1; i >= 0; i--) keys.push(beijingDateKey(-i));
			const costs = keys.map((k) => (s.byDay && s.byDay[k] ? (s.byDay[k].cost || 0) : 0));
			...
			for (let c = 0; c * 7 < cells.length; c++) {
				...
				for (let r = 0; r < 7; r++) {
					...
					const key = keys[di];
					const b = (s.byDay && s.byDay[key]) || EMPTY_BUCKET;
					const lvl = levelOf(b.cost);
					col.push(el('div', {
						className: 'usg-hm-cell' + (lvl > 0 ? ' usg-hm-' + lvl : ''),
						key: r,
						title: key + ' · ' + fmtCost(b.cost) + ' · ' + b.calls + ' 次调用',
					}));
				}
			...
			return el('div', { className: 'usg-hm-wrap' },
				...
				el('div', { className: 'usg-hm-legend' },
					'少',
			...
			return el('div', { className: 'usg-ov' },
				...
				!s
					...
					: el('div', null,
							...
							el('div', null,
								el('div', { className: 'usg-section' }, '近 14 天活跃热力图'),
								heatMap(s, 14),
							),
							el('div', null,
