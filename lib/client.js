...
.usg-card-value.usg-cost { color: var(--dsw-alias-brand-primary); font-size: 18px; }
...
.usg-seg-before { background: var(--dsw-alias-label-tertiary, #888); }
.usg-seg-afterPeak { background: var(--dsw-alias-state-warn-primary); }
.usg-seg-afterOffPeak { background: var(--dsw-alias-state-success-primary); }
...
.usg-chart-days { display: flex; gap: 4px; align-items: stretch; padding: 10px 2px 2px; border: 1px solid var(--dsw-alias-border-l1); border-radius: 8px; background: var(--dsw-alias-bg-layer-2); }
.usg-day-col { flex: 1; display: flex; flex-direction: column; gap: 4px; min-width: 0; }
.usg-day-bar-wrap { display: flex; align-items: flex-end; }
.usg-day-bar { width: 100%; background: var(--dsw-alias-brand-primary); border-radius: 3px 3px 0 0; min-height: 2px; opacity: 0.9; }
.usg-day-bar.usg-zero { background: var(--dsw-alias-border-l2); opacity: 0.6; }
.usg-day-label { font-size: 10px; color: var(--dsw-alias-label-secondary); text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.usg-sbar-fill { height: 100%; border-radius: 4px; background: var(--dsw-alias-brand-primary); opacity: 0.85; }
...
.usg-dock-cost { font-weight: 600; color: var(--dsw-alias-brand-primary); }
...
.usg-ov-stat-value.usg-cost { color: var(--dsw-alias-brand-primary); }
...
`;
		...
		function dayChart(s, height) {
			const dayKeys = Object.keys(s.byDay || {}).sort().slice(-14);
			const maxDayCost = dayKeys.reduce((m, d) => Math.max(m, (s.byDay[d] || EMPTY_BUCKET).cost), 0);
			return el('div', { className: 'usg-chart-days' },
				dayKeys.map((d) => {
					const b = s.byDay[d] || EMPTY_BUCKET;
					const pct = maxDayCost > 0 ? Math.max(2, Math.round((b.cost / maxDayCost) * 100)) : 0;
					return el('div', { className: 'usg-day-col', key: d, title: d + ' · ' + fmtCost(b.cost) + ' · ' + b.calls + ' 次' },
						el('div', { className: 'usg-day-bar-wrap', style: { height: (height || 110) + 'px' } },
							el('div', { className: b.cost > 0 ? 'usg-day-bar' : 'usg-day-bar usg-zero', style: { height: pct + '%' } }),
						),
						el('div', { className: 'usg-day-label' }, d.slice(5)),
					);
				}),
			);
		}
			...
			return el('div', { className: 'usg-ov' },
				...
				!s
					...
					: el('div', null,
							...
							el('div', null,
								el('div', { className: 'usg-section' }, '近 14 天费用'),
								dayChart(s, 80),
							),
							el('div', null,
