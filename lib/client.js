window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DICTS = {
			...
			'band.sepPeak': '9/10后 · 高峰',
			'band.sepOffPeak': '9/10后 · 空闲',
				'model.other': '其他模型',
				'common.unknown': '未知',
				'common.detail': '：{msg}',
				'hm.less': '少',
				...
				'dock.out': '输出',
				'dock.aria': '本会话用量详情',
				'dock.todayBudget': '今日预算已用',
				'dock.todayFigures': '今日 {v}',
				'dock.bandCost': '时段花费',
				'dock.modelUse': '模型用量',
				'dock.cacheHit': '缓存命中',
				'dock.budgetNone': '未设置预算',
				'fab.label': '用量',
				'fab.labelOpen': '用量 ▲',
			...
			'cfg.holidays': '非高峰日（法定节假日与调休上班日，每行一个 YYYY-MM-DD）',
			...
			'cfg.proRoutes': '官方已确认：V4 Pro 继续服务、计费不变（暂不路由到 Flash；若官方恢复路由再开启）',
				'balance.refresh': '刷新余额',
			'balance.runway': '≈ 还可用 {days} 天 · 预计 {date} 耗尽（近 7 天日均 ¥{avg}）',
			},
			en: {
				'band.before': 'Pre-change · legacy',
				'band.afterPeak': 'Post-change · peak',
				'band.afterOffPeak': 'Post-change · off-peak',
			'band.sepPeak': 'Since 9/10 · peak',
			'band.sepOffPeak': 'Since 9/10 · off-peak',
				...
				'dock.in': 'in',
				'dock.out': 'out',
				'dock.aria': 'Session usage details',
				'dock.todayBudget': 'of daily budget used',
				'dock.todayFigures': 'Today {v}',
				'dock.bandCost': 'Cost by period',
				'dock.modelUse': 'Model usage',
				'dock.cacheHit': 'Cache hits',
				'dock.budgetNone': 'no budget set',
				'fab.label': 'Usage',
				'fab.labelOpen': 'Usage ▲',
			...
			'set.alertsSection': 'Recent alerts',
				'set.time': 'Time',
				'set.model': 'Model',
				'set.band': 'Band',
				'set.input': 'Input',
			...
			'cfg.holidays': 'Off-peak days (public holidays & make-up workdays, one YYYY-MM-DD per line)',
			...
			'cfg.proRoutes': 'Official: V4 Pro continues at pro rates (not routed to Flash; enable only if official routing resumes)',
				'balance.refresh': 'Refresh balance',
			'balance.runway': '≈ {days} days left · exhausts ~{date} (7-day avg ¥{avg}/day)',
			},
		};
		...
		function modelLabel(mk) {
			...
			if (mk === 'flash') return 'deepseek-flash';
		...
		const BAND_KEYS = ['before', 'afterPeak', 'afterOffPeak', 'sepPeak', 'sepOffPeak'];
		...
		// 默认汇率（与宿主 defaultConfig 一致）；实际值从 /usage-billing 的 config.usdCny 读取，可在设置中修改
...
.usg-seg-sepPeak { background: #8b5cf6; }
.usg-seg-sepOffPeak { background: #c4b5fd; }
...
.usg-ov-stat { padding: 8px 10px; border: 1px solid var(--dsw-alias-border-l1); border-radius: 8px; background: var(--dsw-alias-bg-layer-2); display: flex; flex-direction: column; gap: 2px; min-width: 0; overflow: hidden; }
.usg-ov-stat-label { font-size: 11px; color: var(--dsw-alias-label-secondary); }
.usg-ov-stat-value { font-weight: 700; font-size: 14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.usg-balance-runway { font-size: 12px; color: #60a5fa; border-top: 1px dashed var(--dsw-alias-border-l1); padding-top: 6px; margin-top: 2px; }
.usg-sbar-click { cursor: pointer; }
.usg-sbar-click:hover { background: var(--dsw-alias-bg-layer-2); border-radius: 6px; }
.usg-alerts { display: flex; flex-direction: column; gap: 6px; }
.usg-alerts-list { display: flex; flex-direction: column; gap: 4px; font-size: 12px; }
.usg-alert-item { display: flex; gap: 10px; color: var(--dsw-alias-label-primary); padding: 3px 6px; border-left: 2px solid #f59e0b; }
.usg-alert-item.usg-alert-over { border-left-color: #ef4444; }
.usg-alert-time { color: var(--dsw-alias-label-secondary); font-variant-numeric: tabular-nums; flex: none; }
...
/* ---------- 本会话用量：触发器 + 详情面板（样式对齐官方 ContextMeter） ---------- */
.usg-sess-root { flex: none; display: inline-flex; }
.usg-sess-trigger { color: var(--dsw-alias-label-tertiary); font-family: inherit; font-size: var(--dsh-content-font-size-secondary, 13px); font-variant-numeric: tabular-nums; line-height: calc(20px + var(--dsh-content-font-delta-secondary, 0px)); white-space: nowrap; cursor: pointer; background: none; border: none; border-radius: 24px; flex: none; align-items: center; gap: 6px; padding: 1px 8px; display: inline-flex; }
.usg-sess-trigger:hover, .usg-sess-trigger[aria-expanded=true] { background: var(--dsw-alias-interactive-bg-hover); color: var(--dsw-alias-label-secondary); }
.usg-sess-trigger .usg-sess-cost { font-weight: 500; color: #60a5fa; }
.usg-sess-track { fill: none; stroke: var(--dsw-alias-border-l3); stroke-width: 2px; }
.usg-sess-fill { fill: none; stroke: var(--dsw-alias-label-tertiary); stroke-width: 2px; stroke-linecap: round; }
.usg-sess-fill-warn { stroke: #f59e0b; }
.usg-sess-fill-over { stroke: #ef4444; }
.usg-sess-anchor { position: fixed; z-index: 1100; }
/* --dsw-specific-menu 自 dsh 0.1.7 起带 alpha（#f8f9fa94），官方同款面板配 backdrop-filter 才不显半透明 */
.usg-sess-panel { box-sizing: border-box; background: var(--dsw-specific-menu); backdrop-filter: var(--dsw-menu-backdrop-filter, blur(40px) saturate(150%)); --dsw-elevation-stroke-color: var(--dsw-alias-border-l1); width: min(300px, 100vw - 24px); box-shadow: var(--dsw-elevation-prominent); color: var(--dsw-alias-label-secondary); cursor: default; border: 0; border-radius: 12px; padding: 12px; font-size: 12px; line-height: 20px; }
.usg-sess-header { align-items: center; gap: 6px; display: flex; }
.usg-sess-figures { font-variant-numeric: tabular-nums; color: var(--dsw-alias-label-primary); margin-left: auto; font-weight: 500; }
.usg-sess-percent { color: var(--dsw-alias-label-primary); font-weight: 500; }
.usg-sess-headline { color: var(--dsw-alias-label-tertiary); }
.usg-sess-bar { background: var(--dsw-alias-interactive-bg-hover); border-radius: 999px; gap: 1px; height: 4px; margin: 10px 0 12px; display: flex; overflow: hidden; }
.usg-sess-segment { background: var(--usg-tint, var(--dsw-alias-label-tertiary)); border-radius: 1px; flex: none; min-width: 2px; height: 100%; }
.usg-sess-rows { margin: 6px 0 0; }
.usg-sess-row { justify-content: space-between; align-items: center; gap: 12px; padding: 2px 0; display: flex; }
.usg-sess-row dt { display: flex; align-items: center; gap: 6px; }
.usg-sess-row dd { font-variant-numeric: tabular-nums; color: var(--dsw-alias-label-primary); margin: 0; }
.usg-sess-swatch { background: var(--usg-tint); border-radius: 2px; width: 8px; height: 8px; display: inline-block; flex: none; }
.usg-sess-tint-messages { --usg-tint: var(--dsw-static-blue-450); }
.usg-sess-tint-tools { --usg-tint: #a78bfa; }
.usg-sess-tint-system { --usg-tint: var(--dsw-static-neutral-bluish-400); }
.usg-sess-div { border-top: 1px solid var(--dsw-alias-border-l1); margin: 8px 0; }
.usg-sess-foot { margin-top: 8px; color: var(--dsw-alias-label-tertiary); font-size: 11px; }
`;
		...
		function fetchStats() {
			return fetch('/usage-billing').then((r) => r.json());
		}
		...
		function sessionBars(s, topN, showTokens) {
			...
			return el('div', { className: 'usg-sbars' },
				top.map((x) => {
					...
					const openable = x.id !== 'unknown' && OPEN_SESSION !== null;
					const rowProps = openable
						? {
								className: 'usg-sbar-row usg-sbar-click',
								title: T('ov.openSession'),
								onClick: () => { try { OPEN_SESSION(x.id); } catch {} },
							}
						: { className: 'usg-sbar-row', title: x.id };
					return el('div', { key: x.id, ...rowProps },
		...
		/** 告警历史（宿主 alerts 缓冲快照，最多 8 条，新在前） */
		function AlertHistory(alerts) {
			if (!Array.isArray(alerts) || alerts.length === 0) return null;
			return el('div', { className: 'usg-alerts' },
				el('div', { className: 'usg-section' }, T('set.alertsSection')),
				el('div', { className: 'usg-alerts-list' },
					alerts.slice().reverse().map((a) => {
						...
						return el('div', { key: a.kind + '-' + a.at, className: 'usg-alert-item' + (a.level >= 2 ? ' usg-alert-over' : '') },
							el('span', { className: 'usg-alert-time' }, fmtDateTime(a.at)),
							el('span', null, text),
						);
					}),
				),
		...
		/** 快速口径的短金额：¥0 / ¥0.53 / ¥12.34（今日卡这类窄卡用） */
		function fmtCostQuick(n) {
			const v = Number(n) || 0;
			if (v === 0) return '¥0';
		...
		/** 今日桶（北京日期 key 对齐 byDay）；无数据返回 null */
		function todayBucket(s) {
			if (!s || !s.byDay) return null;
			const d = new Date(Date.now() + 8 * 3600000);
			const key = d.getUTCFullYear() + '-' + (d.getUTCMonth() + 1 < 10 ? '0' : '') + (d.getUTCMonth() + 1) + '-' + (d.getUTCDate() < 10 ? '0' : '') + d.getUTCDate();
			return s.byDay[key] || null;
		}
		...
		function BalanceCard(balance, onRefresh, onToggle, runway) {
			...
			return el('div', { className: 'usg-balance' },
				...
				runway && runway.days != null
					? el('div', { className: 'usg-balance-runway' },
							T('balance.runway', { days: String(runway.days), date: runway.exhaustDate || '—', avg: (Number(runway.dailyAvg) || 0).toFixed(2) }),
						)
					: null,
			);
		}
		...
		function draftFromConfig(c) {
			...
			return {
				...
				proRoutes: src.proRoutesToFlash === true,
				holidays: Array.isArray(src.holidayDays) ? src.holidayDays.join('\n') : '',
		...
		function buildConfigFromDraft(d) {
			...
			return {
				...
				proRoutesToFlash: d.proRoutes === true,
				holidayDays: String(d.holidays || '').split(/\r?\n/).map((x) => x.trim()).filter((x) => x.length > 0),
			};
		}
		...
		function ConfigEditor(props) {
			...
			const onSave = async () => {
				try {
					...
					const res = await fetch('/usage-billing?action=config', {
						method: 'POST',
			...
			const onReset = async () => {
				try {
					const res = await fetch('/usage-billing?action=config&reset=1', { method: 'POST' });
					applyResult(await res.json());
				} catch (e) {
					setMsg(T('cfg.saveFailed') + T('common.detail', { msg: String(e && e.message ? e.message : e) }));
				}
			...
			return el('details', { className: 'usg-pricing' },
				...
				el('div', { className: 'usg-cfg' },
					...
					el('div', null,
						el('label', { className: 'usg-cfg-check' },
							el('input', { type: 'checkbox', checked: !!draft.proRoutes, onChange: (e) => set('proRoutes', e.target.checked) }),
							el('span', null, T('cfg.proRoutes')),
						),
		...
		const SESS_RADIUS = 5.5;
		const SESS_CIRCUMFERENCE = 2 * Math.PI * SESS_RADIUS;
		/** 时段 → 面板条/图例色（tint 类名对齐官方 ContextMeter 的三色 + 暖色） */
		const SESS_BAND_TINT = {
			before: 'usg-sess-tint-system',
			afterPeak: 'usg-sess-tint-messages',
			afterOffPeak: 'usg-sess-tint-tools',
			sepPeak: 'usg-sess-tint-messages',
			sepOffPeak: 'usg-sess-tint-tools',
		};

		/** 面板里的一行（dt 标签 / dd 数值；headline=true 时作为小节标题行） */
		function sessRow(label, value, headline) {
			return el('div', { className: 'usg-sess-row' },
				el('dt', null, headline ? el('span', { className: 'usg-sess-headline' }, label) : label),
				el('dd', null, value),
			);
		}

		/** 锚定定位：面板 fixed 于触发器上方（对齐官方 useAnchoredPosition 的行为） */
		function useAnchoredTop(rootRef, panelRef, open) {
			...
			react.useEffect(() => {
				...
				const place = () => {
					...
					if (top < margin) top = Math.min(r.bottom + gap, window.innerHeight - ph - margin);
		...
		/** 详情面板：header（本会话花费 + 今日合计）、分时段条、token 明细、模型分布、今日预算 */
		function SessionDetailPanel({ data, s, b, sessionId, fmtMoney, budget }) {
			// 分时段花费（用 recent 中该会话的条目聚合）
			const recents = (s.recent || []).filter((r) => r.sessionId === sessionId);
			const bandAgg = {};
			for (const r of recents) {
				if (!bandAgg[r.band]) bandAgg[r.band] = { cost: 0, calls: 0 };
				bandAgg[r.band].cost += r.cost;
				bandAgg[r.band].calls += 1;
			}
			const bandKeys = Object.keys(bandAgg);
			const bandTotal = bandKeys.reduce((m, k) => m + bandAgg[k].cost, 0);
			// 模型分布
			const modelAgg = {};
			for (const r of recents) {
				const mk = r.modelKey || (typeof r.model === 'string' ? (r.model.indexOf('flash') >= 0 ? 'flash' : r.model.indexOf('pro') >= 0 ? 'pro' : 'other') : 'other');
				if (!modelAgg[mk]) modelAgg[mk] = { calls: 0, cost: 0, name: r.model };
				modelAgg[mk].calls += 1;
				modelAgg[mk].cost += r.cost;
			}
			const tb = todayBucket(s);
			const daily = budget && budget.daily && budget.daily.limit != null ? budget.daily : null;
			const dailyPct = daily && daily.limit > 0 ? Math.min(999, Math.round((daily.spent / daily.limit) * 100)) : null;
			// 官方口径命中率：hit / 输入总量（input 已含缓存命中部分）；input=0 时用 hit+miss 兜底
			const hitRate = b.input > 0 ? Math.round((b.cacheHit / b.input) * 100)
				: (b.cacheHit + b.cacheMiss > 0 ? Math.round((b.cacheHit / (b.cacheHit + b.cacheMiss)) * 100) : null);
			// 分段条：时段占比（无数据则单段预算进度）
			const segments = bandTotal > 0
				? bandKeys.map((k) => ({ key: k, tint: SESS_BAND_TINT[k], width: (bandAgg[k].cost / bandTotal) * 100 }))
				: (dailyPct != null ? [{ key: 'total', tint: null, width: Math.min(100, dailyPct) }] : []);
			return el('div', {
				role: 'dialog',
				'aria-label': T('dock.aria'),
				className: 'usg-sess-panel',
			},
				el('div', { className: 'usg-sess-header' },
					el('span', { className: 'usg-sess-headline' }, T('dock.used')),
					el('span', { className: 'usg-sess-percent' }, fmtMoney(b.cost)),
					tb ? el('span', { className: 'usg-sess-figures' }, T('dock.todayFigures', { v: fmtCostQuick(tb.cost) })) : null,
				),
				el('div', { className: 'usg-sess-bar' },
					segments.filter((x) => x.width > 0).map((seg) =>
						el('div', { key: seg.key, className: 'usg-sess-segment' + (seg.tint ? ' ' + seg.tint : ''), style: { width: seg.width + '%' } }))),
				el('dl', { className: 'usg-sess-rows' },
					sessRow(T('dock.calls'), String(b.calls)),
					sessRow(T('dock.in'), fmtTokens(b.input)),
					sessRow(T('dock.cacheHit'), fmtTokens(b.cacheHit) + (hitRate != null ? ` (${hitRate}%)` : '')),
					sessRow(T('dock.out'), fmtTokens(b.output)),
				),
				bandKeys.length > 0 ? el('div', { className: 'usg-sess-div' }) : null,
				bandKeys.length > 0 ? el('dl', { className: 'usg-sess-rows' },
					sessRow(T('dock.bandCost'), '', true),
					...bandKeys.map((k) => sessRow(
						el('span', { className: 'usg-sess-dt-swatch' },
							el('span', { className: 'usg-sess-swatch' + (SESS_BAND_TINT[k] ? ' ' + SESS_BAND_TINT[k] : ''), 'aria-hidden': true }),
							bandLabel(k)),
						fmtMoney(bandAgg[k].cost) + ' · ' + bandAgg[k].calls)),
				) : null,
				Object.keys(modelAgg).length > 1 ? el('div', { className: 'usg-sess-div' }) : null,
				Object.keys(modelAgg).length > 1 ? el('dl', { className: 'usg-sess-rows' },
					sessRow(T('dock.modelUse'), '', true),
					...Object.keys(modelAgg).map((k) => sessRow(modelLabel(modelAgg[k].name || k), modelAgg[k].calls + ' · ' + fmtMoney(modelAgg[k].cost))),
				) : null,
				el('div', { className: 'usg-sess-div' }),
				el('dl', { className: 'usg-sess-rows' },
					sessRow(T('dock.todayBudget'), dailyPct != null ? dailyPct + '%' : T('dock.budgetNone')),
				),
			);
		}
		...
		function DockReadout(props) {
			...
			const position = useAnchoredTop(rootRef, panelRef, open && !!s && !!sessionId);
			// Esc / 外点关闭（对齐官方 useDismissOnOutsidePointer）
			react.useEffect(() => {
				if (!open) return;
			...
			const budget = data && data.budget ? data.budget : null;
			const daily = budget && budget.daily && budget.daily.limit != null ? budget.daily : null;
			const tb = todayBucket(s);
			// ring：今日预算进度；未设预算时画本会话花费占今日合计的比例；全无则空环
			let ringPct = null;
			if (daily && daily.limit > 0) ringPct = Math.min(100, (daily.spent / daily.limit) * 100);
			else if (tb && tb.cost > 0) ringPct = Math.min(100, (b.cost / tb.cost) * 100);
			const ringCls = ringPct == null ? '' : ringPct >= 100 ? ' usg-sess-fill-over' : ringPct >= 80 ? ' usg-sess-fill-warn' : '';
			const fmtMoney = moneyFormatter((data && data.currency) || 'cny', data && data.config ? data.config.usdCny : DEFAULT_USD_CNY);
			return el('span', { className: 'usg-sess-root', ref: rootRef },
				el('button', {
					type: 'button',
					className: 'usg-sess-trigger',
					'aria-label': T('dock.aria'),
					...
					el('svg', { viewBox: '0 0 14 14', width: '14', height: '14', 'aria-hidden': true },
						el('circle', { className: 'usg-sess-track', cx: '7', cy: '7', r: SESS_RADIUS }),
						el('circle', {
							className: 'usg-sess-fill' + ringCls, cx: '7', cy: '7', r: SESS_RADIUS,
							strokeDasharray: `${SESS_CIRCUMFERENCE * (ringPct || 0) / 100} ${SESS_CIRCUMFERENCE}`,
							transform: 'rotate(-90 7 7)',
						}),
					),
					T('dock.used'),
					el('span', { className: 'usg-sess-cost' }, fmtMoney(b.cost)),
					'· ' + b.calls + ' ' + T('dock.calls'),
					'· ' + T('dock.in') + ' ' + fmtTokens(b.input),
					'· ' + T('dock.out') + ' ' + fmtTokens(b.output),
				),
				open && el('div', { ref: panelRef, className: 'usg-sess-anchor', style: position || { visibility: 'hidden', left: 0, top: 0 } },
					el(SessionDetailPanel, { data, s, b, sessionId, fmtMoney, budget })),
			);
		}
		...
		function cardModel(s, sessionId) {
			...
			if (byModel.flash && byModel.flash.calls > 0) return 'deepseek-flash';
		...
		function FooterButton(props) {
			...
			react.useEffect(() => {
				if (!READ_CURRENT) return;
				const read = () => {
					try { setSessionId(READ_CURRENT.get()); } catch { setSessionId(null); }
				};
				read();
				return READ_CURRENT.subscribe(read);
			}, []);
			...
			react.useEffect(() => {
				...
				const poll = () => {
					fetchStats().then((d) => {
						...
						// 控制台也打一条，便于事后确认触发过
						try { console.log('[usage-billing] budget alert:', fresh.map((t) => t.text).join(' | ')); } catch {}
					...
					!s
						...
						: el('div', null,
								el('div', { className: 'usg-ov-stats' },
									(function () { const tb = todayBucket(s); return tb ? ovStat(T('ov.today') + ' · ' + tb.calls + ' ' + T('ov.todayCalls'), fmtCostQuick(tb.cost), 'usg-cost') : ovStat(T('ov.sessions'), String(sessionsCount)); })(),
								...
								BalanceCard(data ? data.balance : null, null, null, data ? data.runway : null),
								el('div', null,
		...
		function UsagePanel() {
			...
			const refreshBalance = () => {
				fetch('/usage-billing?action=balance', { method: 'POST' }).then(() => refresh()).catch(() => {});
			};

			const toggleBalance = (enabled) => {
				const cfg = (data && data.config) ? data.config : null;
				if (!cfg) return;
				fetch('/usage-billing?action=config', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
			...
			const onBackfill = async () => {
				setBackfilling(true);
				try {
					const res = await fetch('/usage-billing?action=backfill', { method: 'POST' });
			...
			const onClear = async () => {
				...
				setConfirming(false);
				try {
					await fetch('/usage-billing?action=clear', { method: 'POST' });
			...
			const head = el('div', { className: 'usg-head' },
				...
				el('div', { className: 'usg-actions' },
					...
					el('button', { className: 'usg-btn', onClick: () => download('/usage-billing?action=export&format=csv&scope=day') }, T('export.csvDay')),
					el('button', { className: 'usg-btn', onClick: () => download('/usage-billing?action=export&format=csv&scope=session') }, T('export.csvSession')),
					el('button', { className: 'usg-btn', onClick: () => download('/usage-billing?action=export&format=json') }, T('export.json')),
			...
			return el('div', { className: 'usg-panel' },
				...
				TrendBars(s),
				BalanceCard(data.balance, refreshBalance, toggleBalance, data.runway),
				el('div', null,
		...
		function apply(ctx) {
			...
			if (locale) {
				ctx.effect(() => locale.register('dsh-usage-billing', DICTS), 'usage-billing: locale dicts');
			...
			// 会话导航：dsh 0.1.6+ 由 uiWorkspace.openSession 负责（sessions.open 已移除）；
			// 旧版 dsh 回退 sessions.open。当前会话号：0.1.6+ 从 uiWorkspace.selection 读取，
			// 旧版从 sessions.list 快照的 current 字段读取。
			UIWORKSPACE = null;
			try {
				const uiws = ctx.get('uiWorkspace');
				if (uiws && typeof uiws.openSession === 'function') UIWORKSPACE = uiws;
			} catch { UIWORKSPACE = null; }
			OPEN_SESSION = null;
			if (UIWORKSPACE) OPEN_SESSION = (id) => UIWORKSPACE.openSession(id);
			else if (SESSIONS && typeof SESSIONS.open === 'function') OPEN_SESSION = (id) => SESSIONS.open(id);
			READ_CURRENT = null;
			if (UIWORKSPACE && UIWORKSPACE.selection) {
				const sel = UIWORKSPACE.selection;
				if (typeof sel.getSnapshot === 'function' && typeof sel.subscribe === 'function') {
					READ_CURRENT = {
						get: () => { const s = sel.getSnapshot(); return s && typeof s.sessionId === 'string' ? s.sessionId : null; },
						subscribe: (fn) => sel.subscribe(fn),
					};
				}
			}
			if (!READ_CURRENT && SESSIONS && SESSIONS.list) {
				READ_CURRENT = {
					get: () => {
						try {
			...
			class Boundary extends react.Component {
				...
				componentDidCatch(error, info) {
					try { console.error('[usage-billing] render error:', error, info); } catch {}
				}
				render() {
					if (this.state.error) {
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register(
				{ name: "settings.section", id: "usage-billing", order: 25, label: () => T('set.sectionLabel') },
			...
			ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register(
				{ name: "sidebar.footer.action", id: "usage-billing", order: 10, label: () => T('fab.label') },
				(props) => guard(react.createElement(FooterButton, { wide: props.wide })),
			));

			// 主界面 · 悬浮汇总面板 + 预算告警 toast
			ctx.slots.inject("shell.overlay", () => ctx.slots.register(
				{ name: "shell.overlay", id: "usage-billing-panel", order: 10 },
				() => guard(el('div', null,
